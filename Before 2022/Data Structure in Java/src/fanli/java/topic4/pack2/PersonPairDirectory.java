package fanli.java.topic4.pack2;
...
public class PersonPairDirectory {
	...
	// The problem here is that we are trying to perform a narrowing conversion
	// without
	// an explicit cast. Namely, the value returned by method findOther is of
	// type Person
	// while the variable cute_one, to which it is assigned, is of the narrower
	// type Student, a class implementing interface Person, Thus, we use a cast
	// to convert
	...
	// Student
	// works fine as long as we are sure that the call to myDirectory.findOther
	// is
	// really giving us a Student object. In general, interfaces can be a
	// valuable
	// tool for the design of general data structures, which can then be
	// specialised
